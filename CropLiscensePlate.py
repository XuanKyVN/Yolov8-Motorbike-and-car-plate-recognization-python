...
def imgwrite(img):
    ...
    cv2.imwrite(os.path.join(r"C:\Users\Admin\PythonLession\YoloV8\yolov8-detect-count-crop-save-main\cropimg",filename), img)
while True:    
    ret,frame = cap.read()
    if not ret:
        break
...
#            cv2.imshow(str(id),crop) 
    ...
    if cv2.waitKey(1)&0xFF==27:  # 27 la nut Enter
        break
cap.release()
